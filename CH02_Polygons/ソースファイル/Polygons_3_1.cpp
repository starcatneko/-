...
XMMATRIX CreateWorldMatrix( void )
{
	...
	//最初は回転するとわかりづらいので、
	//このXMMatrixIdentityを呼ぶことで単位行列(何もしない行列)を返し
	//回転を防止しています。いったん指定通りに描画できたら以下のXMMatrixIdentityの行は消してください。
...
int DrawChangingPictures( void )
{
	...
	fAngleDelta = 2.0f * PI / float(div);
	fAngle1 = 0.0f;
	fAngle2 = fAngleDelta;

	//ひとまずこのループでやってほしいことは錐形(四角錐)の側面の三角形を分割数(div)分作って
	//表示することである。
	//
	//※現状だと、XとZが全て0であるので、画面上には何も表示されません
	//適切な頂点情報を指定することで、三角形を描画してください
	//
	//錐形の中心が0,0,0であるとして、半径(中心から底面頂点までの距離)が１だとするときの
	//それぞれの座標を入力してやる。
	//できた人は、分割数を増やして円錐にしてください。
	for ( i = 0; i < div; i++ ) {
		//以下のDraw3DPolygonは定数指定しているため四角錐になっているが、
		//sinやcosを利用することで四角錐にしてみてください。
		//
		Draw3DPolygon( 0, -1.0f,  0, 0xffff0000,//コーン周辺①(cos,-1,sin)
				0, -1.0f,  0, 0xff00ff00,//コーン周辺②(cos,-1,sin)
				0.0f,  1.0f,                 0.0f, 0xff0000ff );//コーン中心(0,1,0)
		
		
		//ここまでできた人は、四角錐の底面に蓋をしてみてください。
		//教科書の85ページを参考にしながら…
		//でも教科書からも挑戦的な宿題が出てますね…無駄ポリゴンを減らすには
		//どうしたらいいか考えてください。割と簡単ではあるんですけどね
		//
		//ここで僕の個人的な経験として、ポリゴンで円を表現したときに1方向にグラデをかけたいという要望が
		//あって、それに対応するには点の結び方を工夫しなければならない…
		//思いついたのはZの字に並べていくって方法
		
		fAngle1 += fAngleDelta;
		fAngle2 += fAngleDelta;
	}
	//もし…ここまでが楽ちんにできちゃった人は、
	//錐形ではなく円柱型を作ってみよう。
