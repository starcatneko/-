...
///回転の合成をしてもらいます。
///自動でY軸中心回転を行っています。
///これを上下でX軸中心回転できるようにしてください。
///それが出来たら左右で平行移動できるようにしてください。
///
///戻り値として返した行列が三角形に適用されると思ってください。
XMMATRIX CreateWorldMatrix( void )						// ワールド行列の生成
{
	...
	//平行移動行列作成
	matTrans_X = XMMatrixTranslation(fOffsetX,0,0);

	//掛け算に注意してきちんと合成してみましょう。
	return matRot_Y;						// 回転の合成
}
