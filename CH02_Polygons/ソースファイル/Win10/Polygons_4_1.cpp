...
// 緯度θ=fTheta、経度φ=fPhi
// 今回やって欲しいことは、rとθとφからX,Y,Zを計算して
// vResult.x,y,zに代入して欲しい。さぁ、やってくれたまえ
//詳しくは教科書の90ページを見るよろし
//X-Z平面はφでY軸回りに回転し、θは縦方向に回転するものだと思っておいてください。
//最初は真っ暗ですが、スッ…とみかんの実のひとつみたいなのが出ます。
//その後はDrawChangingPicturesに細工して、ループを追加し、球体にしてみてください。
XMFLOAT3 ConvertRTPToXYZ( float r, float fTheta, float fPhi )
{
	XMFLOAT3	vResult;

	vResult.x = r*sin(fTheta)*cos(fPhi);
	vResult.y = r*cos(fTheta);
	vResult.z = r*sin(fTheta)*sin(fPhi);
...
int DrawChangingPictures( void )						// 画像の描画
{
	...
	for ( i = 0; i < CORNER_NUM / 2; i++ ) {
		p1 = ConvertRTPToXYZ( R, fTheta1, fPhi1 /5);	// 頂点1
		p2 = ConvertRTPToXYZ( R, fTheta1/10, fPhi2*2 );	// 頂点2
		p3 = ConvertRTPToXYZ( R, fTheta2/5, fPhi1/5 );	// 頂点3
		p4 = ConvertRTPToXYZ( R, fTheta2*10, fPhi2*2 );	// 頂点4
