...
int RenderScanLine( Vector2  p1, Vector2  p2, Vector2  p3)
{
	int				x, y;
	float				fTemp;
	...
	// 下準備(y座標でソート)
	//yが小さい順に並ぶようにしておく。
	//ソートしておけば、位置関係を気にせずY座標を決めれるから。
	//逆に言うと、ソートしていない場合だと、yを増やせばいいのか減らせばいいのかわからない。
	//さて、Y座標でソートするのだから…y1<y2<y3になるようにするには…わかるな？
	if ( p1.y > p2.y ) {
		 fTemp = p1.y;
		p1.y = p2.y;
		p2.y = fTemp;
	...
	// y方向に対する傾きを計算(デフォルト0にしとくので自分で傾きを考えてね)
	//これも二年生の時に勉強した「傾き」をしっかり考えてください。
	//y=ax+bのaってどうやって計算してましたか？a=Y増加分/X増加分でしたね？
	//さぁ頑張って考えよう
	a12 = ((p2.x-p1.x) /   ( p2.y - p1.y));
	a13 = ((p3.x - p1.x) / (p3.y - p1.y));
	a23 = ((p3.x - p2.x) / (p3.y - p2.y));

	//最初の座標はp1(一番上)から
	xl = p1.x;  xr = p1.x;
	//プログラムの構造上FlushDrawingPicturesを呼び出さないと正しく描画されない
	if ( a12 < a13 ) {//左凸三角形
		for ( y = ( int )p1.y; y < ( int )p2.y; y++ ) {		// y方向ループ
		...
		for ( y = ( int )p2.y; y < ( int )p3.y; y++ ) {		// y方向ループ
				for ( x = ( int )xl; x < ( int )xr; x++ ) {	// x方向ループ
					DrawPoints( x, y, 0, 255, 255 );
				}
...
//    g_pImmediateContext->IASetPrimitiveTopology( D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST );
	...
	RenderScanLine(Vector2(200,50),Vector2(50,300),Vector2(500,200));
