...
int CheckHit( XMFLOAT3 *pv3Point )
{
	float			fAngle;
	int			nAngleIndex;
	XMFLOAT3		v3LineVec;//外辺のベクトル
	XMFLOAT3		v3HitVec;//外辺ベクトル起点から、対象点へのベクトル
	float			fCross;
	float			fDot;							// 内積

	
	//ガタガタを解消するには押し戻し処理を内部的に２回行い、
	//１回の押し戻しでまた「めり込んで」いるなら押し戻し方向に問題がある。
	//その場合はそのまま戻す前に戻す位置を補正する。
	//やり方としては「内積」が辺からハミ出していたら、ハミダサナイヨウニ補正(0～辺の長さ)してください

	
	
	//ひとまずはX軸とのなす角度を計算します。
	//あとでこの計算結果を「インデックス」に変換します
	//なおatan2は(Y,X)でX軸からの角度を計算して返す関数です。
	fAngle = atan2f( pv3Point->z - g_HitPolygonCenter.z,
					 pv3Point->x - g_HitPolygonCenter.x );

	//計算結果は-π～πとなるので、マイナスの値の時には２πを足して０～２πにします。
	if ( fAngle < 0.0f ) fAngle += 2.0f * PI;

	//０～2πを0～CHECK_POLYGON_ANGLESのインデクス値に変換
	nAngleIndex = ( int )( fAngle / ( 2.0f * PI ) * CHECK_POLYGON_ANGLES );

	// 各辺のベクトルと、点へのベクトルの外積を取る。
	// 外積の値がプラスかマイナスかで内側にあるのか外側にあるのかを判断する。
	// xz平面二次元のベクトルとして定義すると外積は
	// Ax*Bz-Az*Bxとなるため、これがプラスかマイナスかで判断する。
	// 基本的には(２次元の場合は)、順方向にベクトルを掛け算するとプラス、逆だとマイナスになる
	// 
	...
	if(fCross > 0.0f){//当たってたら補正処理を行います
		//やり方は…わかるよな？
		//辺のベクトルを正規化したものに、射影長をかけるのだ…
		//そのベクトルを辺起点に足せばいい…わかったか！！
	...
	float fHitCircle = (pv3Point->x - g_HitPolygonCenter.x)* (pv3Point->x - g_HitPolygonCenter.x)+ (pv3Point->z - g_HitPolygonCenter.z)* (pv3Point->z - g_HitPolygonCenter.z);
	float circleRadias = 1 * 1;
	if (fHitCircle <= circleRadius)
	{

	}
	return nAngleIndex;//該当インデックスを返します
}
