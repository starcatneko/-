...
float CheckGroundHeight( MY_PLAYER *pPlayer )
{
	...
	if ( ( fBlockRes_x + fBlockRes_z ) < 1.0f ) {//左下
		...
		fGrad_z = ( g_fHeights[nBlockIndex_z + 1][nBlockIndex_x]
				  - g_fHeights[nBlockIndex_z][nBlockIndex_x] );
		fHeight = g_fHeights[ nBlockIndex_z ][ nBlockIndex_x ]
				+ fGrad_x * fBlockRes_x + fGrad_z * fBlockRes_z;//高さ情報を計算して、代入しましょう。
		nInIndex = 0;
	}else {//右上
		...
		fGrad_z = ( g_fHeights[nBlockIndex_z][nBlockIndex_x + 1]
				  - g_fHeights[nBlockIndex_z + 1][nBlockIndex_x + 1] );
		fHeight = g_fHeights[ nBlockIndex_z + 1 ][ nBlockIndex_x + 1 ]
				+ fGrad_x * ( 1.0f - fBlockRes_x ) + fGrad_z * ( 1.0f - fBlockRes_z );//高さ情報を計算して、代入しましょう。
		nInIndex = 1;
	}
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "課題3 1601291 西岡豊" ),
