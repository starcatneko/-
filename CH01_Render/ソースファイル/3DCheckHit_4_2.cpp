...
float CheckGroundHeight( MY_PLAYER *pPlayer )
{
	...
	fGrad_x = (g_fHeights[1]-g_fHeights[0])*fPlayerBlock_x;//X方向の勾配を出す
	
	fPlayerBlock_z = ( pPlayer->v3Pos.z + ( GROUND_SIZE / 2 ) ) / ( float )GROUND_SIZE;
	fGrad_z = (g_fHeights[2] - g_fHeights[0])*fPlayerBlock_z;//X方向の勾配を出す
	;//Z方向の勾配を出す

	return g_fHeights[0]+ fGrad_x+ fGrad_z;//適切な高さを返す(XZ勾配からバイリニアする)
}
