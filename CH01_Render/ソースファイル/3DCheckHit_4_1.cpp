...
// 高さデータ
float			g_fHeights[HEIGHT_NUM] = { 0.0f, 1.2f, 1.6f, 8.4f, 0.0f, 0.8f, 2.2f, 4.4f, 0.8f, 0.6f, 0.0f };


// 地面の高さチェック
float CheckGroundHeight( MY_PLAYER *pPlayer )
{
	float		fPlayerBlock;//プレイヤーがいるブロック(小数点あり)
	int		nBlockIndex;//プレイヤーが居るブロックから少数点を切り捨てたもの
	float		fBlockRes;//プレイヤーが居るブロックの小数点部分
	float		fGrad;	// 傾き
	fPlayerBlock = ( pPlayer->v3Pos.x + ( GROUND_SIZE / 2 ) ) / ( float )BLOCK_WIDTH;
	nBlockIndex = (int)fPlayerBlock;//プレイヤーブロックインデックスの小数点部分を切り捨てろ
	...
	fBlockRes = fPlayerBlock - (int)fPlayerBlock;//プレイヤーブロックインデックスの小数点部分を取り出す

	fGrad = (g_fHeights[nBlockIndex] - g_fHeights[nBlockIndex + 1])*fBlockRes;//勾配を計算してください(今のインデックスから次のインデックスまでの勾配);
	

	return g_fHeights[nBlockIndex]- fGrad;//現在の高さを返してください
}
