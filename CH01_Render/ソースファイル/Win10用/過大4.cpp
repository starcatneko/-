...
{
	// ここに射角の計算を書いてください			
	float fSin = fDistance * GR / (fVel_0 * fVel_0);
...
int MoveShell( void )
{
	...
	if ( Shell_1.bActive ) {
		...
		if ( ( Shell_1.v3Pos.y < SHELL_RADIUS ) && ( Shell_1.v3Vel.y < 0.0f ) ) {
			...
			//InitTatget();
			Shell_1.v3Vel.y *= -0.5f;
			//Shell_1.v3Vel.y = (Player_1.fTheta)/(1+bound);
		}
		if (bound > 16)
		{
			bound = 0;
...
//	int						i;
	...
	g_mmShell.mMatrix.r[3].m128_f32[1] = 0.2f;
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "課題4 1601291_西岡豊" ),
