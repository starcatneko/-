...
//nBright3 左下画素値
//nBright4 右下画素値
//隣の画素を滑らかに補間するために「Bilinearフィルタ」という技法を用います。
//デフォルト状態では補間が行われていないために教科書55ページの理論を元に
//FilterBiLinear関数を完成させてください。6_2.exeと同様の結果になれば完成です。
int FilterBiLinear( int nBright1, int nBright2, int nBright3, int nBright4,
				    float lu, float lv )
{
	//ここにバイリニアフィルタの式を書く。
	//それぞれの4点をluおよびlvをもとに線形補間を二回かけることにより
	//バイリニアフィルタとする
	//ちなみに左上がnBright1、右上がnBright2、左下がnBright3、右下がnBright4とする

	return nBright1 ;//とりあえず授業の都合上上左上の画素をそのまま返している。
}
...
int RenderScanLine( void )
{
	...
	vBase1_x = 0.1f * cosf( fAngle );				// 基底ベクトル1
	vBase1_y = 0.1f * sinf( fAngle );
	vBase2_x = 0.1f * cosf( fAngle + PI / 2.0f );	// 基底ベクトル2
	vBase2_y = 0.1f * sinf( fAngle + PI / 2.0f );
