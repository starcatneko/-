...
int RenderScanLine( void )
{
	...
	int					nBright=0;
	a++;
	//とりあえずコメントアウトしている部分を非コメント化するとグラデになります。
	//今回の課題は、この部分を書き換えて「斜め」のグラデにしてもらうことです。
	//傾きも変えたりしてみましょう。
	//中学2年くらいの数学でやれる部分なので頑張りましょう。
	//斜めにできたらのこぎり波を試し、そして三角波にしてください
	//それができたら飽和加算(一定値を超えたらずっとその値にする)してみてください。
	//つまり本日の課題は
	//①グラデ
	//②斜めに
	//③のこぎり波に
	...
	for ( y = 0; y <= VIEW_HEIGHT; y++ ) {		// y方向ループ
		for ( x = 0; x <= VIEW_WIDTH; x++ ) {	// x方向ループ
			nBright = abs((((x+y - b)*2) & 0x1ff)- 256);// 明るさがy座標に連動
			//nBright = fmax(fmin((x+y-180)/2,255),0);// 明るさがy座標に連動
			//												 //ビット演算が分からない人は
			//nBright = y % 255;でもいい
			//nBright - b;
			DrawPoints( x, y, nBright, 0, nBright);
		}
...
#endif
    // コンパイル
    hr = D3DCompileFromFile( _T( "Basic_2D_Geom.fx" ), nullptr,
		nullptr, "VS", "vs_4_0_level_9_1",
								dwShaderFlags, 0,  
		&pVertexShaderBuffer,
		&pError );
    if ( FAILED( hr ) ) {
