...
int RenderScanLine( void )
{
	...
	float				fAngle = PI / 12.0f;			// 回転角
	int r = rand()%4+1;
	vBase1_x = 0.2*cos(fAngle);		// 基底ベクトル1
	vBase1_y = 0.2*-sin(fAngle);
	vBase2_x = 0.2*sin(fAngle);		// 基底ベクトル2
	vBase2_y = 0.2*cos(fAngle);
	//真ん中中心で回転させるためにやってます。つまるところこのbx,byが回転の中心になってます。
	//見れば分かるように幅と高さの半分を割り当てています。
	...
	for ( y = 0; y < VIEW_HEIGHT; y++ ) {		// y方向ループ
		...
		for ( x = 0; x < VIEW_WIDTH; x++ ) {	// x方向ループ
			...
			//以下のコードは１ピクセル横に進めるのと同じ意味です(回転後のな)
			cx += vBase1_x;
			cy += vBase1_y;
		}
		FlushDrawingPictures();
		//以下のコードは１ピクセル下に進めるのと同じ意味です(回転後のな)
		bx += vBase2_x;
		by += vBase2_y;
	}
...
HRESULT MakeShaders( void )
{
...
#endif
    // コンパイル
    hr = D3DCompileFromFile( _T( "Basic_2D_Geom.fx" ), NULL, NULL, "VS", "vs_4_0_level_9_1",
    ...
    hr = D3DCompileFromFile( _T( "Basic_2D_Geom.fx" ), NULL, NULL, "PS", "ps_4_0_level_9_1",
