...
{
	const float SCALE_MIN = 0.1f;				// 最小拡大率
	const float SCALE_MAX = 2.0f;				// 最大拡大率
	int					x, y;
	...
	for ( y = 0; y < VIEW_HEIGHT; y++ ) {		// y方向ループ
		for ( x = 0; x < VIEW_WIDTH; x++ ) {	// x方向ループ
			//x,yで指定した位置から色の情報を取得します。
			//&nRed等でアドレス放り込んで、中で値が入っているわけね。
			//さて、今回は原寸で表示させておりますが、これを「縦横倍」で表示してください
			//GetBMPPixelはピクセルの「色」を取得する関数ですが、画像を「倍」の大きさにするならば
			//「色を打つ座標」と「色をとってくる座標」はどういう関係になっているのかを考えて、画像を倍にしてみよう。
			//①2倍にしてみよう
			//②半分にしてみよう
			//③時間によって拡大したり縮小したりしてみよう
			//④拡大縮小の中心点を左上から、ウィンドウ中心にしてみよう

			GetBMPPixel(x/(VIEW_WIDTH/2), y / (VIEW_HEIGHT / 2), &mbPicture,&nRed, &nGreen, &nBlue );		// 点取得
			

			//x,yで指定した位置に色を置きます。
			//nRed,nGreen,nBlueは色の三原色の赤緑青ね
