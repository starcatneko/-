...
int RenderScanLine( void )
{
	...
	const float			fAmbient = 64.0f;		// アンビエント光
	const float			fDirect = 192.0f;		// ディレクショナル光
	float				fDot;					// 内積
	cnt++;
	//lx = (float)(lightx - 128) / 256.f;
	//レイトレーシングのパチもん実装をします。
	//そもそもカメラの定義もスクリーンの定義もありません
	//レイトレーシングを勉強したことがある人なら「あれ？」って実装です。
	//今回は輝度をベクトルの内積で計算するとそれっぽくなるという事を学んでください。
	...
	for ( y = 0; y < VIEW_HEIGHT; y++ ) {		// y方向ループ
		for ( x = 0; x < VIEW_WIDTH; x++ ) {	// x方向ループ
			...
			//dxとdyからdzを計算します。
			dzsq = RAD*RAD - dx*dx - dy*dy;// zの2乗計算(教科書の23ページを参照)
			
			if ( dzsq > 0.0f ) {
				dz = sqrtf( dzsq );//dxとdyとRADから計算したdzを算出
				//原点中心なのでそれぞれdx,dy,dzが中心から球体の表面までのベクトル成分になります
				nx = dx / RAD;  ny = dy / RAD;  nz = dz / RAD;	// 法線ベクトル
				//法線ベクトルと光源方向ベクトルとの内積をとってください。
				fDot = lx*nx + ly*ny + lz*nz;// 内積
				
				//内積の値を元にランバートの余弦則から明るさを計算してください
				if ( fDot >= 0.0f ) {			// 光源からの光が当たる
					//内積値に一定の数値をかけてディフューズ成分を計算してください
					//定数であるアンビエントを足してください。
					//ひとまずそこまでやってみた後で擬似スペキュラをやってください
					//31ページを参考にしながらホニャララ乗してください。
					//キラーンって感じになると思います。
					//それができたら模様をつけましょう。教科書の33ページを参考に
					//XZ平面上における角度を計算し、その角度によって色が変わるようにしてみてください。
					//
					float fAngle = 0;
					if (fAngle < 0.0f)fAngle += 2.0f*PI;
					nBright = (int)fAmbient+fDot*fDirect;//
					fAngle = atan2f(dz, dy);
					if ((int)(fAngle / (2.0f*PI) * 64) & 1)
					{
						r = 1;
					}
				}
				...
				DrawPoints( x, y, nBright, nBright * r, 0 );
			}
		}
...
//        { "TEXTURE",  0, DXGI_FORMAT_R32G32_FLOAT,       0, 32, D3D11_INPUT_PER_VERTEX_DATA, 0 },
    ...
    DirectX::XMMATRIX		mScreen;
    mScreen = DirectX::XMMatrixIdentity();
...
int DrawPoints( int x, int y, int nRed, int nGreen, int nBlue )
{
	g_cvVertices[g_nVertexNum].v4Pos   = DirectX::XMFLOAT4( ( float )x, ( float )y, 0.0f, 1.0f );
	g_cvVertices[g_nVertexNum].v4Color = DirectX::XMFLOAT4( nRed / 255.0f, nGreen / 255.0f, nBlue / 255.0f, 1.0f );
	g_nVertexNum++;
...
//	int				i, j;
    // 画面クリア
	DirectX::XMFLOAT4	v4Color = DirectX::XMFLOAT4( 0.0f, 0.0f, 1.0f, 1.0f );
