...
XMMATRIX CreateWorldMatrix( MY_PLAYER *pPlayer )
{
	...
	///3軸を決定する
	v3Up = pPlayer->v3Normal;//地面の法線方向がそのまま入っている(140行目くらいを参照のこと)
	v3Foward = XMFLOAT3( cosf( pPlayer->fAngleXZ ), 0.0f, sinf( pPlayer->fAngleXZ ) );
	v3Side = Cross( &v3Foward, &v3Up );//外積の結果は２つのベクトルに直交するベクトル
	v3Side = Normalize( &v3Side );//正規化
	v3Foward = Cross( &v3Up, &v3Side );//更にそいつと外積取る
	matResult = XMMatrixIdentity();				// 単位行列に
	
	//ここにモデル回転のためのプログラムを書いてください
	//特定の方向に向かせるにはそれぞれのベクトルをそれぞれの行に入れてください
	//P242を参照のこと
