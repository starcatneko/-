...
XMMATRIX MakeBillboardMatrix( XMMATRIX *pmatView, XMFLOAT3 *pv3Pos )
{
	...
	// 回転部分の転置行列作成
	// 行列そのものは2-2と同じでいい。
	// 今回はそれよりもアルファブレンディングが問題であるので、そこも修正する。
	// D3D11_DEPTH_WRITE_MASKで検索しましょう。
...
HRESULT InitD3D( void )
{
	...
	//爆発エフェクトの時を思い出しましょう。
	//Zバッファに書き込まないという設定はどうするんでしたっけ？
	dsDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL;//ALLでは書き込んでしまいます
...
//	g_pImmediateContext->OMSetDepthStencilState( g_pDSDepthState, 1 );
    ...
    // ラスタライザの設定(時計回りカリング)
	ZeroMemory( &drd, sizeof( drd ) );//←ZeroMemoryは嫌いなんじゃああああ！！！
	drd.FillMode				= D3D11_FILL_SOLID;
