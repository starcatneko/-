...
int MakeGroundNormals( CUSTOMVERTEX *pVertices, WORD *pIndices, int nIndexNum )
{
	...
	//ループが3ずつ進んでいるのは１つの面を構成しているのが3つの頂点だからだ。
	//
	for ( i = 0; i < nIndexNum; i += 3 ) {
		...
		//v1にp1→p3のベクトルを
		//v2にp1→p2のベクトルを設定してくれ
		v1 = XMFLOAT3();
		v2 = XMFLOAT3();

		//v3Normalにv1とv2の外積を代入して正規化(標準化)してくれ
		
		
		//↑で計算した法線情報の結果は「面」の法線なので、関係する頂点全てに法線値を加算してください。
		//まず、X方向インデックスとY方向インデックスを出します。nIndex1から計算します。
		//あとはインデックスごとに３つ計算します。
		//インデクスが出たら、そのNormalに↑Normalを加算します。
	}
