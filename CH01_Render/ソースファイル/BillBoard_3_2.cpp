...
{
	XMMATRIX		matBill = XMMatrixIdentity();							// ビルボード行列

																			// 回転部分の転置行列作成
	matBill.r[0].m128_f32[0] = pmatView->r[0].m128_f32[0];
	matBill.r[0].m128_f32[1] = pmatView->r[1].m128_f32[0];
	matBill.r[0].m128_f32[2] = pmatView->r[2].m128_f32[0];
	matBill.r[1].m128_f32[0] = pmatView->r[0].m128_f32[1];
	matBill.r[1].m128_f32[1] = pmatView->r[1].m128_f32[1];
	matBill.r[1].m128_f32[2] = pmatView->r[2].m128_f32[1];
	matBill.r[2].m128_f32[0] = pmatView->r[0].m128_f32[2];
	matBill.r[2].m128_f32[1] = pmatView->r[1].m128_f32[2];
	matBill.r[2].m128_f32[2] = pmatView->r[2].m128_f32[2];

	matBill.r[3].m128_f32[0] = pv3Pos->x;
	matBill.r[3].m128_f32[1] = pv3Pos->y;
	matBill.r[3].m128_f32[2] = pv3Pos->z;
	return matBill;
}


//なんでこの作者は「参照」使わへんのかな(´・ω・｀)面倒でしゃーない
// ビルボードのZソート
//@param pSource	ビルボードソート用配列(in)
//@param ppSort		ビルボードソート用配列(out)
//@param nDataNum	データの数(配列要素数)
//@param pmView		ビュー行列
				   ...
				   int nDataNum, XMMATRIX *pmView )
{
	...
	//ソートキーの計算
	//カメラZ方向の距離毎にソートするためにまずはソートキーを計算する
	//ソートキーはカメラからの距離である。
	//キーであるZ値が大きい方から描画します
	for ( i = 0; i < nDataNum; i++ ) {
		//それぞれの座標にビュー行列をかけることによって座標変換(カメラ行列)後のZ座標を計算
		//Zのみを算出しています(カメラのZベクトルにそれぞれの座標を内積している→カメラZ方向の座標を得られる)
		//ちなみにカメラZベクトルとは、行列の3列目のことです。
		//①座標をビュー行列で変換する
		//	座標と、Zベクトル(3列目)との内積をとる←Z値
		//②そのZ値を計算する
		//③ソートキーに入れる
		pSource[i].fSortKey = pSource[i].v3Pos.x*pmView->r[0].m128_f32[2]+
			pSource[i].v3Pos.y*pmView->r[1].m128_f32[2]+
			pSource[i].v3Pos.z*pmView->r[2].m128_f32[2];		// View座標でのz成分
	}

	//計算したソートキーを元に実際のソートを行う。
	//以下はインサーションソートである。一般的にあまり効率はよくない
	//が、こういう場面ではそれなりの速度が出る。
	for ( i = 0; i < nDataNum; i++ ) {
		for ( j = i - 1; j >= 0; j-- ) {
...
HRESULT MakeShaders( void )
{
...
#endif
	// コンパイル
	hr = D3DCompileFromFile(_T("Basic_3D_TexMark.fx"), NULL, NULL, "VS", "vs_4_0_level_9_1",
	...
	hr = D3DCompileFromFile(_T("Basic_3D_TexMark.fx"), NULL, NULL, "PS", "ps_4_0_level_9_1",
...
HRESULT InitGeometry( void )
{
	...
	hr = LoadTexture( _T( "12.png" ), &g_tBillboardTexture, 321, 339, 512, 512 );
    if ( FAILED( hr ) ) {
