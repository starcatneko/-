...
HRESULT MakeShaders( void )
{
...
#endif
    // コンパイル
    hr = D3DX11CompileFromFile( _T( "Basic_3D_Light.fx" ), nullptr, nullptr, "VS", "vs_4_0",
    ...
    hr = D3DX11CompileFromFile( _T( "Basic_3D_Light.fx" ), nullptr, nullptr, "PS", "ps_4_0",
								dwShaderFlags, 0, &pPixelShaderBuffer, &pError );
    if ( FAILED( hr ) ) {
	char* e=new char [pError->GetBufferSize()];
	memcpy(e,pError->GetBufferPointer(),pError->GetBufferSize());
...
int FlushDrawingPictures( void )
{
	...
	if ( ( g_nVertexNum > 0 ) /*&& g_pNowTexture*/ ) {
		...
		if ( SUCCEEDED( hr ) ) {
			CopyMemory( mappedResource.pData, &( g_cvVertices[0] ), sizeof( CUSTOMVERTEX ) * g_nVertexNum*2 );
