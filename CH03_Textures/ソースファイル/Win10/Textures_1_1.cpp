...
// テクスチャマッピングを行う
// テクスチャを貼るためには、各ポリゴンに
// どのようにテクスチャを貼るかを確定するための
// uv座標を指定する必要がある
// uv座標は1つ1つの頂点について、その頂点が
// 絵の中のどの部分に相当するかを確定することによって、
// 絵の貼られ方を一意に決定する
// ただし、uv座標の座標値はドット単位ではないことに注意が必要
// uv座標は、貼り付けるテクスチャの大きさに関係なく、
// テクスチャの端から端までが0～1となるような座標で指定する
// そうすることによって、実際のテクスチャの解像度に関わりなく
// 同じようにテクスチャを貼り付けることができるようにしている
...
HRESULT InitGeometry( void )
{
	...
	hr = LoadTexture( _T( "6.png" ), &g_tTexture, 480, 480, 512, 512 );
    if ( FAILED( hr ) ) {
...
// テクスチャは最終的にピクセルシェーダで
// 実際にどう貼られるのかが決定されるため、
// ピクセルシェーダに貼り付けるべきテクスチャの絵を
// 渡してあげる必要がある
// ピクセルシェーダは。この関数に渡された貼り付けるべき絵と、
// バーテックスシェーダから流れてきたuv座標から、
// どのテクセルをどのようにサンプルするかを決定する
