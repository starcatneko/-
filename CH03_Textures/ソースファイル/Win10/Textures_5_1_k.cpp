...
{
	int					i, j;
	CUSTOMVERTEX		Vertices[( CORNER_NUM + 1 ) * ( CORNER_NUM / 4 + 1 )];	// 頂点データ
	WORD				wIndices[CORNER_NUM * CORNER_NUM / 4 * 2 * 3];	// インデックスデータ
	float				fTheta;
	float				fPhi;
	float				fAngleDelta;
...
ID3D11DepthStencilState *g_pDSStencilState2 = NULL;	// Zバッファのステート
...
ID3D11BlendState		*g_pbsNormalBlend;		// ふつうブレンド
